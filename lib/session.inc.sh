...
# SESSIONS
# 
# Sessions allow you to share data between another script instance.
# Sessions are separated between system users. 
# Session data are stored in:
# ${_session_rootdir)/$(session_sha1_hash)/<session_name>
# 
#*/
...
function session_init() {
    ...
    SESSION_NAME=${USER}_$(/usr/bin/basename $0})
...
#/**
# Set the session variable. The content is store in the one file without special characters like new line.
# 
# @param    String  $1          Variable name.
...
#/**
# Set the session variable. It includes special character lik new line, so that allow store multiline data.
# 
# @param    String  $1          Variable name.
# @param    String  $2          Variable value.
#*/
...
#/**
# Add another data content to the existing session variable (or create a new variable).
# 
# @param    String  $1          Variable name.
...
# 
# @param    String  $1          Variable name.
# @param    String  $2          Default value. If the variable not exist, it will be create using default value.
#*/
...
#/**
# Wypisz listę zmiennych sesyjnych from the group.
# 
...
function _session_rootdir() {
    { echo /tmp/ram00/$(/usr/bin/whoami)/sessions; } 2> /dev/null
}
...
#    eval $_SESSION
    ...
    case ${method} in
        set) echo "${data}" > ${SESSION_DIR}/${path} 
             ;;

        put) echo -e "${data}" > ${SESSION_DIR}/${path}
             ;;

        push) echo -e "${data}" >> ${SESSION_DIR}/${path}
              ;;
